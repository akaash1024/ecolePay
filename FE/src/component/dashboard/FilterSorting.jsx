    ...
    <>
      <div className="bg-white border-2 border-green-300 rounded-lg p-6 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {uniqueSchools.map((school) => (
                <button type="button"
                  ...
                  className={`px-3 py-1 rounded-full text-sm border transition-colors ${
                    schoolFilter.includes(school)
                      ? "bg-green-500 text-white border-green-500"
                      : "bg-white text-gray-700 border-gray-300 hover:border-green-500"
                  }`}
                >
                  {school.split("_")[1]} (
                  {transactionSet.filter((t) => t.school_id === school).length})
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded">
          ...
          {filteredAndSortedTransactions.length !== transactionSet.length &&
            ` (filtered from ${transactionSet.length} total)`}
        </div>
      </div>
    </>
  );
